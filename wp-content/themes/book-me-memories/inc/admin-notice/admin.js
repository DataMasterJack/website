( function( $ ){
    $( document ).ready( function(){
      $( '.flight-booking-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( flight_booking_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'flight_booking_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
