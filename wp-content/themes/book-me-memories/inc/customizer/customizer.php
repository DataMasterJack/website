	...
	function illdy_customize_register( $wp_customize ) {
		...
		$wp_customize->add_section(
			new Book Me Memories_Customize_Section_Pro(
				...
				'illdy-pro-section',
				array(
					'title'    => esc_html__( 'Book Me Memories', 'illdy' ),
					'pro_text' => esc_html__( 'Documentation',         'illdy' ),
					'pro_url'  => 'https://bookmememories.com/wp/support/illdy/',
					'priority' => 0
				)
			)
		);
...
if ( ! function_exists( 'illdy_customizer_js_load' ) ) {
	function illdy_customizer_js_load() {
		...
		$Book Me MemoriesCustomizer['sections'] = illdy_get_sections_position();
		...
		wp_localize_script( 'illdy-customizer', 'Book Me MemoriesCustomizer', $Book Me MemoriesCustomizer );
